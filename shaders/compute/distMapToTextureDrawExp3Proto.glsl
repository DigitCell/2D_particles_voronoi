...
float Voronoi(in ivec2 coord,in int selfIdx) {
    ...
    float d0 = distance(coord,pi0);
    float d1 = distance(coord,pi1);
    float d2 = distance(coord,pi2);

    float df =min(.4/(1./max(d1 - d0, .001) + 1./max(d2 - d0, .001)), 1.);
    df*= smoothstep(radius*1.8,0. ,d0);
...
void main() {
  ...
  vec3  col = pal2( (pointData[centerIndex].force.x)/170.0f, vec3(0.5,0.5,0.5),vec3(0.5,0.5,0.5),vec3(1.0,1.0,1.0),vec3(0.0,0.33,0.67) )*c.x;

  // Apply the bump - or a powered variation of it - to the color for a bit of highlighting.
  vec2 c2 = hMap(ivec2(coord)+ 2, selfIdx);
  float b =  (c2.x - c.x)*0.5;
