...
*/
    ...
    for (int gridY = -2; gridY < 3; gridY++)
    {
        ...
        if(gridX==0 && gridY==0) continue;
    		int xLookup = int(closestCandidate.x) + gridX;
    		int yLookup = int(closestCandidate.y) + gridY;
        ...
        {
          float otherDistance =length(otherCandidate-coord);
          if (abs(otherDistance-closestDistance)>2.0 )
            {
              findCandidateBorder=true;
              closestDistanceBorder=closestDistance;
            }
        }
    	}
    }
